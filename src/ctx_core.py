...
class CtxCore:
    ...
    def add_remote(self, url: str, name: str = "origin") -> OperationResult:
        ...
        try:
            ...
            if name in existing_remotes:
                return OperationResult(False, error=f"Remote '{name}' already exists")
            ...
            return OperationResult(False, error=f"Error adding remote: {e}")
        except Exception as e:
            return OperationResult(False, error=f"Error adding remote: {e}")

    def list_remotes(self) -> OperationResult:
        """List all configured remotes

        Returns:
            OperationResult with list of remotes and their URLs
        """
        if not self.is_ctx_repo():
            return OperationResult(False, error="Not in a ctx repository")
        ...
        try:
            ...
            for remote in repo.remotes:
                remotes.append({
                    ...
                    'fetch_url': list(remote.urls)[0] if remote.urls else '',
                    'push_url': list(remote.urls)[0] if remote.urls else ''
                })

            if not remotes:
                return OperationResult(True, "No remotes configured", data=[])

            return OperationResult(
                True,
                f"Found {len(remotes)} remote(s)",
                data=remotes
            )

        except Exception as e:
            return OperationResult(False, error=f"Error listing remotes: {e}")

    def sync_remote(self, remote_name: str = "origin") -> OperationResult:
        """Pull then push to remote (basic sync)

        Args:
            remote_name: Name of the remote to sync with (default: 'origin')

        Returns:
            OperationResult with sync information
        """
        if not self.is_ctx_repo():
            return OperationResult(False, error="Not in a ctx repository")
        ...
        try:
            ...
            if remote_name not in remote_names:
                return OperationResult(
                    False,
                    error=f"Remote '{remote_name}' not found. Available remotes: {', '.join(remote_names) if remote_names else 'none'}"
                )

            remote = repo.remotes[remote_name]
            current_branch = self.get_current_branch()

            # Check if we have any changes that need to be committed first
            if repo.is_dirty():
                return OperationResult(
                    False,
                    error="Repository has uncommitted changes. Please commit or discard changes before syncing."
                )
            ...
            try:
                ...
                sync_info['pulled'] = True
                sync_info['pull_info'] = str(pull_info[0].flags) if pull_info else "up-to-date"

            except GitCommandError as e:
                if "merge conflict" in str(e).lower() or "conflict" in str(e).lower():
                    sync_info['conflicts'] = True
                    return OperationResult(
                        False,
                        error=f"Pull resulted in merge conflicts. Please resolve conflicts manually and try again.",
                        data=sync_info
                    )
                else:
            ...
            try:
                ...
                if "rejected" in str(e).lower():
                    return OperationResult(
                        False,
                        error=f"Push rejected. The remote may have changes that need to be pulled first.",
                        data=sync_info
                    )
                else:
                    return OperationResult(
            ...
            message = f"Synced with remote '{remote_name}'"
            if sync_info.get('pull_error'):
                message += f" (pull had issues: {sync_info['pull_error']})"
            ...
            return OperationResult(False, error=f"Error syncing with remote: {e}")